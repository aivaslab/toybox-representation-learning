...
def layer_eval(args):
	...
	feat_depth = [64, 128, 256, 512]

	if args['num_layers_frozen'] == 1:
		network_forward_func = network.forward_l1
		layer_feat_reduced = int(round(np.sqrt(np.asarray([int(fc_in / feat_depth[0])]))[0]))
		featSize = layer_feat_reduced * layer_feat_reduced * feat_depth[0]
		pooling_layer = nn.Sequential(nn.AdaptiveMaxPool2d(layer_feat_reduced), nn.BatchNorm2d(feat_depth[0]), nn.Flatten())
	elif args['num_layers_frozen'] == 2:
		network_forward_func = network.forward_l2
		layer_feat_reduced = int(round(np.sqrt(np.asarray([int(fc_in / feat_depth[1])]))[0]))
		featSize = layer_feat_reduced * layer_feat_reduced * feat_depth[1]
		pooling_layer = nn.Sequential(nn.AdaptiveMaxPool2d(layer_feat_reduced), nn.BatchNorm2d(feat_depth[1]), nn.Flatten())
	elif args['num_layers_frozen'] == 3:
		network_forward_func = network.forward_l3
		layer_feat_reduced = int(round(np.sqrt(np.asarray([int(fc_in / feat_depth[2])]))[0]))
		featSize = layer_feat_reduced * layer_feat_reduced * feat_depth[2]
		pooling_layer = nn.Sequential(nn.AdaptiveMaxPool2d(layer_feat_reduced), nn.BatchNorm2d(feat_depth[2]), nn.Flatten())
	else:
		network_forward_func = network.forward_l4
		layer_feat_reduced = int(round(np.sqrt(np.asarray([int(fc_in / feat_depth[3])]))[0]))
		featSize = layer_feat_reduced * layer_feat_reduced * feat_depth[3]
		pooling_layer = nn.Sequential(nn.AdaptiveMaxPool2d(layer_feat_reduced), nn.BatchNorm2d(feat_depth[3]), nn.Flatten())

	pooling_layer = pooling_layer.cuda()
	if args['dataset'] == "cifar10":
		trnsfr_classifier = nn.Sequential(nn.Linear(featSize, 10)).cuda()
		trainTransform = transforms.Compose([transforms.ToPILImage(),
											 transforms.Resize(224),
											 transforms.RandomHorizontalFlip(p = 0.5),
											 transforms.RandomCrop(size = 224, padding = 5),
		...
		trnsfr_classifier = nn.Sequential(nn.Linear(featSize, 100)).cuda()
		trainTransform = transforms.Compose([transforms.ToPILImage(),
											 transforms.Resize(224),
											 transforms.RandomHorizontalFlip(p = 0.5),
											 transforms.RandomCrop(size = 224, padding = 5),
		...
		trnsfr_classifier = nn.Sequential(nn.Linear(featSize, 1000)).cuda()
		trainTransform = transforms.Compose([transforms.ToPILImage(),
											 transforms.Resize(224),
											 transforms.RandomHorizontalFlip(p = 0.5),
											 transforms.RandomCrop(size = 224, padding = 5),
	...
	for ep in range(numEpochs):
		...
		for idx, (_, images, targets) in enumerate(tqdmBar):
			...
			images = images.cuda()
			targets = targets.to(torch.device('cuda:0'))
			layer_out = network_forward_func(images)
			feats = pooling_layer(layer_out)
			logits = trnsfr_classifier(feats)
			loss = nn.CrossEntropyLoss()(logits, targets)
			avg_loss = (avg_loss * (b - 1) + loss.item()) / b
			loss.backward()
		...
		if ep % args['decay_epochs'] == args['decay_epochs'] - 1 and ep > 0:
			...
			for idx, (_, images, labels) in enumerate(trainDataLoader):
				with torch.no_grad():
					layer_out = network_forward_func(images.cuda())
					feats = pooling_layer(layer_out)
					logits = trnsfr_classifier(feats)
				top, pred = utils.calc_accuracy(logits, labels.cuda(), topk = (1,))
				top1acc += top[0].item() * pred.shape[0]
