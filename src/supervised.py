"""Module to perform supervised learning experiments"""
import torch
import torchvision
import numpy as np
...
def train(network, classifier, train_data, test_data, args):
    ...
    print("Train", len(train_data), utils.online_mean_and_sd(trainLoader))
    print("Test", len(test_data), utils.online_mean_and_sd(testLoader))
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer,
                                                     T_max=max(
                                                         10, (args['epochs'] - 10)) * len(trainLoader), eta_min=0.001)
...
if __name__ == "__main__":
    ...
    test_transform = transforms.Compose([transforms.ToPILImage(), transforms.Resize(224), transforms.ToTensor(),
                                         transforms.Normalize(mean, std)])
