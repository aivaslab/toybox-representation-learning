...
def learn_unsupervised(args, simclrNet, device, devices):
	...
	for ep in range(numEpochs):
		...
		for _, images, _ in tqdmBar:
			...
			feat1, feat2, feat3 = features[:bSize], features[bSize:2*bSize], features[2*bSize:]
			# logits, labels = info_nce_loss(features = features, dev = device, temp = args["temperature"])
			loss = loss_fn(feat1, feat2, feat3)
...
def learn_supervised(args, simclrNet, device, k, devices):
	transform_train = get_train_transform(args["transform"])
...
def train_unsupervised_and_supervised(args):
	...
	learn_unsupervised(args = args, simclrNet = network, device = device, devices = deviceIDs)
	...
	print(pytorch_total_params, pytorch_total_params_train)
	learn_supervised(args = args, simclrNet = network, device = device, k = 1, devices = deviceIDs)
