...
from pose_loader_toybox import pose_data_toybox
import parser
...
def learn_unsupervised(args, simclrNet, devices):
	...
	if args["dataset"] == "core50":
		trainData = pose_data_core50(root = "./data", rng = args["rng"], train = True, nViews = 2, size = 224,
								transform = transform_train, fraction = args["frac1"],
								hyperTune = args["hypertune"], split_by_sess = args["sessionSplit"],
								distortArg = args["distortArg"], gap = args['gap'])
	else:
		trainData = pose_data_toybox(root = "./data", rng = args["rng"], train = True, nViews = 2, size = 224,
								transform = transform_train, fraction = args["frac1"],
								hyperTune = args["hypertune"], distortArg = args["distortArg"], gap = args['gap'])
	...
	optimizer.add_param_group({'params': simclrNet.fc2.parameters()})

	scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = int(1.6 * numEpochs),
														   eta_min = 0.1*args["lr"])
	train_losses = []
	if args['loss'] == 'cosine':
		loss_fn = utils.TripletMarginWithDistanceLoss(distanceFunction = lambda x, y: 1.0 - F.cosine_similarity(x, y),
											   margin = args['margin'])
	else:
	...
	for ep in range(numEpochs):
		...
		for _, images, _ in tqdmBar:
			...
			if show:
				...
				im2.show()
				im3 = transforms.ToPILImage()(unorm(images[2 * args['batch_size']]))
				im3.show()
				show = False

			features, poseFeat = netParallel(images)
			bSize = int(images.shape[0] / 3)
			feat1, feat2, feat3 = poseFeat[:bSize], poseFeat[bSize:2*bSize], poseFeat[2*bSize:]
			logits, labels1, labels2 = utils.triple_info_nce_loss(features = features, temp = args["temperature"])
			if ratio > 0:
				loss = nn.CrossEntropyLoss()(logits, labels1)
				loss += nn.CrossEntropyLoss()(logits, labels2)
			else:
				loss = 0

			if ratio < 1.0:
				loss2 = loss_fn(feat1, feat2, feat3)
			else:
				loss2 = 0
			loss = loss * ratio + loss2 * (1 - ratio)
