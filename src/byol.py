...
def learn_unsupervised(args, network, device):
	...
	print(network.encoder_prediction)
	...
	for ep in range(numEpochs):
		...
		for _, (images1, images2), _ in tqdmBar:
			...
			if show:
				...
				im3 = transforms.ToPILImage()(images1[0])
				im3.show()
				im4 = transforms.ToPILImage()(images2[0])
				im4.show()
			...
			std1 = torch.std(features1, dim = 0).cpu()
			tot_std += torch.mean(std1)
			avg_std = tot_std / b
			...
			tqdmBar.set_description("Epoch: {:d}/{:d}, Loss: {:.6f}, LR: {:.8f}, b: {:.4f}, std: {:.5f}".format(ep + 1, numEpochs, avg_loss,
																						optimizer.param_groups[0][
																							'lr'], network.beta, avg_std))
