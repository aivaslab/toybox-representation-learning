...
class pose_data_toybox(pose_data):
	...
	def __init__(self, root, rng, train = True, transform = None, nViews = 2, size = 224, split =
				"unsupervised", fraction = 1.0, hyperTune = True, frac_by_object = False,
				 distortArg = False, interpolate = True, gap = 1):
		...
		super().__init__(root = root, rng = rng, train = train, transform = transform, nViews = nViews, size = size,
						 ...
						 frac_by_object = frac_by_object, distortArg = distortArg, gap = gap)
...
if __name__ == "__main__":
	...
	print(len(simclr))
	'''
	for i, (idx, images, _) in enumerate(trainDataLoader):
		if i == 0:
			print(i, len(images[0]))
			for j in range(len(images[0])):
				unorm = utils.UnNormalize(mean = mean, std = std)
				im1 = transforms.ToPILImage()(unorm(images[0][j]))
				im2 = transforms.ToPILImage()(unorm(images[1][j]))
				im3 = transforms.ToPILImage()(unorm(images[2][j]))
				dst = Image.new('RGB', (3*im1.width, im1.height))
				dst.paste(im1, (0, 0))
